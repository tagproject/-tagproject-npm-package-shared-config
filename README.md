<p align="center"><img width="150" src="https://cdn.jsdelivr.net/gh/tagproject/art/packages/standard-shared-config/icon.svg" alt="Changelog-guru logo"></p>

<p align="center">
    <a href="https://github.com/tagproject/ts-package-shared-config/actions"><img src="https://github.com/tagproject/ts-package-shared-config/actions/workflows/build.yml/badge.svg" alt="Build Status"></a>
    <a href="https://www.npmjs.com/package/@tagproject/ts-package-shared-config"><img alt="npm" src="https://img.shields.io/npm/v/@tagproject/ts-package-shared-config.svg"></a>
    <a href="https://www.npmjs.com/package/@tagproject/ts-package-shared-config"><img alt="NPM" src="https://img.shields.io/npm/l/@tagproject/ts-package-shared-config.svg"></a>
     <a href="https://github.com/keindev/standard-shared-config"><img src="https://img.shields.io/badge/standard--shared--config-nodejs%2Bts-green?logo=data:image/png;base64," alt="Standard Shared Config"></a>
</p>

[Standard Shared Config](https://github.com/keindev/standard-shared-config) for `tagproject` packages with **Node.js** and **TypeScript**

## Install

```console
npm install @tagproject/ts-package-shared-config
```

## Usage

- Add `"prepare:config": "ts-package-shared-config"` to `scripts` property in your `package.json`
- Run `npm run prepare:config`
